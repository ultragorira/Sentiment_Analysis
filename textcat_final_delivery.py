...
OUTPUT_FILENAME = "TelusAI_TextCategorization_Final_Delivery.json"
...
def read_customer_file(filepath):
    ...
    sorted_customer_content = sorted(
        customer_content, key=lambda entry: (entry["topic_id"], entry["text_id"])
    )
    ...
    for key, entries in grouped_customer_content:
        topic_first_posts[key] = f"{key}_{list(entries)[0]['text_id']}"
...
for customer_entry in read_customer_file(customer_file):
    item_key = f'{customer_entry["topic_id"]}_{customer_entry["text_id"]}'
    ...
    customer_entry["theme"] = item_results(item)
